function res_gam = gen_gam_test_inpaper2(beta,lambda,sigma,Ns,ts,Nsim)
...
crivalmono40 = betainv(0.40,parmono(1), parmono(2)); 
...
crivalum60 = betainv(0.60,parum(1), parum(2)); 
crivalum75 = betainv(0.75,parum(1), parum(2)); 
crivalum100 = 1;

% Ns=50;
crivalbe5   = exp(-2.3555-1.3600*log(Ns));
crivalbe10 = exp(-1.5112-1.3702*log(Ns));
% options = optimset('Display','off','Simplex','on',...
%                     'TolFun',.000001, 'Algorithm','active-set');
% par = fmincon(@(par) betafit(par,crivalbe5,crivalbe10), [0.8884 71.5457], ...
%      [],[],[],[],[0.001 0.001],[],[],options);
% betacdf(crivalbe5,par(1),par(2))
% betacdf(crivalbe10,par(1),par(2))
parbe = [0.8884 71.5457];
crivalbe25 = betainv(0.25,parum(1), parum(2)); 
crivalbe50 = betainv(0.50,parum(1), parum(2)); 
crivalbe75 = betainv(0.75,parum(1), parum(2)); 
crivalbe100 = 1;
...
monorpexe25 = zeros(N,1);
monorpexe50  = zeros(N,1);
monorpexe75 = zeros(N,1);
...
% 6, exponential using data regarding whatever larger than median as
% censored
% calculate the median
medi   = (lambda^(2)*gaminv(0.5,lambda^(-2),1))^(sigma/lambda)*exp(beta);
expar3 = (sum(exprd(find(exprd<=medi)))+length(find(exprd>medi))*medi)...
    /length(find(exprd<=medi));
qexp_medi = exp(-ts/expar3);
...
pexeoutbe    = RPEXEv1('EventTime',exprd,'Censor',ones(Ns,1),'Trend',0);
...
% 21, rpexe estimate monotone, p-value = 0.4
...
% 22, rpexe estimate umbrella, down and up, p-value = 0.6
if min(pexeoutum.pvalues) < crivalum60, % if the change point is found
    tchange = pexeoutum.times(find(pexeoutum.pvalues < crivalum60));
    [qumrpexe60 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qumrpexe60 = qexp_raw;   
end


% 23, rpexe estimate, p-value = 0.1
if min(pexeoutbe.pvalues) < crivalbe10, % if the change point is found
    tchange = pexeoutbe.times(find(pexeoutbe.pvalues < crivalbe10));
    [qberpexe10 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qberpexe10 = qexp_raw;
end

% 24, rpexe estimate betone, p-value = 0.25
if min(pexeoutbe.pvalues) < crivalbe25, % if the change point is found
    tchange = pexeoutbe.times(find(pexeoutbe.pvalues < crivalbe25));
    [qberpexe25 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qberpexe25 = qexp_raw;   
end

% 25, rpexe estimate betone, p-value = 0.5
if min(pexeoutbe.pvalues) < crivalbe50, % if the change point is found
    tchange = pexeoutbe.times(find(pexeoutbe.pvalues < crivalbe50));
    [qberpexe50 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qberpexe50 = qexp_raw;   
end

% 26, rpexe estimate betone, p-value = 0.75
if min(pexeoutbe.pvalues) < crivalbe75, % if the change point is found
    tchange = pexeoutbe.times(find(pexeoutbe.pvalues < crivalbe75));
    [qberpexe75 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qberpexe75 = qexp_raw;   
end

% 27, rpexe estimate betone, p-value = 1.00
if min(pexeoutbe.pvalues) < crivalbe100, % if the change point is found
    tchange = pexeoutbe.times(find(pexeoutbe.pvalues < crivalbe100));
    [qberpexe100 lamest] = pexeest(exprd, ones(Ns,1), tchange, ts);
else 
    qberpexe100 = qexp_raw;   
end
...
berpexe10(iter) = qberpexe10;
berpexe25(iter) = qberpexe25;
berpexe50(iter) = qberpexe50;
berpexe75(iter) = qberpexe75;
berpexe100(iter) = qberpexe100;
...
rmspeexp_medi = sqrt(sum((exp_medi-quanval)'*(exp_medi-quanval)/N));
...
rmspeumrpexe60 = sqrt(sum((umrpexe60-quanval)'*(umrpexe60-quanval)/N));
rmspeberpexe10 = sqrt(sum((berpexe10-quanval)'*(berpexe10-quanval)/N));
rmspeberpexe25 = sqrt(sum((berpexe25-quanval)'*(berpexe25-quanval)/N));
rmspeberpexe50 = sqrt(sum((berpexe50-quanval)'*(berpexe50-quanval)/N));
rmspeberpexe75 = sqrt(sum((berpexe75-quanval)'*(berpexe75-quanval)/N));
rmspeberpexe100 = sqrt(sum((berpexe100-quanval)'*(berpexe100-quanval)/N));
...
res_gam = [[var(kme_raw) mean(kme_raw)-quanval rmspekme_raw max(abs(kme_raw-quanval)) mean(abs(kme_raw-quanval))]
        ...
        [var(exp_medi) mean(exp_medi)-quanval rmspeexp_medi max(abs(exp_medi-quanval)) mean(abs(exp_medi-quanval))]
        ...
        [var(umrpexe60) mean(umrpexe60)-quanval rmspeumrpexe60 max(abs(umrpexe60-quanval)) mean(abs(umrpexe60-quanval))]
        [var(berpexe10) mean(berpexe10)-quanval rmspeberpexe10 max(abs(berpexe10-quanval)) mean(abs(berpexe10-quanval))]
        [var(berpexe25) mean(berpexe25)-quanval rmspeberpexe25 max(abs(berpexe25-quanval)) mean(abs(berpexe25-quanval))]
        [var(berpexe50) mean(berpexe50)-quanval rmspeberpexe50 max(abs(berpexe50-quanval)) mean(abs(berpexe50-quanval))]
        [var(berpexe75) mean(berpexe75)-quanval rmspeberpexe75 max(abs(berpexe75-quanval)) mean(abs(berpexe75-quanval))]
        [var(berpexe100) mean(berpexe100)-quanval rmspeberpexe100 max(abs(berpexe100-quanval)) mean(abs(berpexe100-quanval))]
        ];
